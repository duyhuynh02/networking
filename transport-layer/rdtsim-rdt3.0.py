...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.state = 0                          # 0 to send, 1 to wait 
        self.seqnum = 0 
        self.acknum = 0 
        self.checksum = 0 
        self.acknum_expected = 0 
        self.packet_to_retransfer = None 
        self.time_out = 10.0 

    # Called from layer 5, passed the data to be sent to other side.
    # The argument `message` is a Msg containing the data to be sent.
    def output(self, message):
        if self.state == 0: #ready to transfer message 
            self.checksum = self.compute_checksum(message)
            # print('checksum: ', self.checksum)
            packet = self.create_packet(message.data)
            print('packet: ', packet)
            self.packet_to_retransfer = packet 
            to_layer3(self, packet)
            self.state = 1 # wait for ack 
    ...
    def compute_checksum(self, message):
        total = sum([ord(c) for c in message.data.decode('utf-8')])
        checksum = self.seqnum + self.acknum + total 
        return checksum 

    def create_packet(self, payload):
        packet = Pkt(self.seqnum, self.acknum, self.checksum, payload)
        return packet
...
class EntityB:
    ...
    def compute_checksum(self, message):
        total = sum([ord(c) for c in message.decode('utf-8')])
        checksum = self.seqnum + self.acknum + total 
        return checksum 
